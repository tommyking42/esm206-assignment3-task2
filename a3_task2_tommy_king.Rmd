---
title: "Assignment 3 Task 2: Follow me: more data wrangling tools"
...
```{r}
# Your code here: 
sw_split_name <- starwars %>% #starting with star wars data 
  separate(col = name,  # separating the column 'name'
           into = c("name_1", "name_2"), #into two columns
           sep = "-") #at the first dash
...
```{r}
# Your code here:
sw_skin_sep <- starwars %>% # new dataset from the starwars data
  separate(col = skin_color, # separating skin_color column
           into = c("skin_1", "skin_2"),  # into two new columns
           sep = ",",  # separated by a comma
           extra = "merge", # merge everything after first column 
...
```{r}
# Your code here: 
mpg_model_class <- mpg %>%  # new dataset from mpg data
  unite(col = model_class, # new column called 'model_class'
        ...
        remove = FALSE) # retains original columns
...
# trying the ymd() function with my birthday in various formats
```
...
```{r}
# Your code here: 
some_dates <- c(05041999, 06011996, 06201964)
some_dates_converted <- mdy(some_dates)
some_dates_converted

class(some_dates_converted)

# converting some random dates to mdy() format and checking their classification
```
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>% #new dataset from lakers
  filter(player == "Pau Gasol", type == "hook") %>% # filtering by player and type for Pau Gasol and hookshot
  mutate(date_new = ymd(date)) %>% # creating a new date column
  mutate(month_new = month(date_new)) %>%  # creating a new month column
  
count(month_new) # counting number of hookshots per month
...
```{r}
# Your code here: 
storms_danger <- storms %>%  # creating a new dataset from storms data
  mutate(danger = case_when( # new column 'danger' when...
    status == "hurricane" ~ "high", # if status is hurricane, then return danger of high
    status != "hurricane" ~ "moderate" # if status does not equal hurricane, then return moderate
  ))
